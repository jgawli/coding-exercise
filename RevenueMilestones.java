...
public class RevenueMilestones {
    ...
    int[] getMilestoneDays(int[] revenues, int[] milestones) {
        ...
        for (int i = 0; i < milestones.length; i++) {
            int milestoneIdx = getMilestoneIndex(revenues, milestones[i]);
            output[i] = milestoneIdx;
        }

        return output;
    }

    int getMilestoneIndex(int[] revenues, int milestone) {
        int currentMilestone = 0;

        for (int i = 0; i <revenues.length; i++) {
            currentMilestone = currentMilestone + revenues[i];
